"""M2 production NUTS — φ fixed at 1.0, reparam A (log_R0 + simplex), γ from registry.

Settings: 500 warmup + 1000 sample × 4 chains sequential. Total 6000 iters.

Outputs:
- outputs/calibration/m2_prod_posterior.nc  (idata, primary)
- outputs/calibration/m2_prod_samples.npz   (raw grouped samples, fallback)
- outputs/calibration/m2_prod_result.json   (per-chain verdict)
- outputs/calibration/M2_PROD_DONE.flag

Run detached:
    nohup caffeinate -i -s uv run python scripts/m2_production.py \\
        > outputs/calibration/m2_prod.log 2>&1 & disown
"""
...
SENTINEL = OUTDIR / "M2_PROD_DONE.flag"
RESULT_JSON = OUTDIR / "m2_prod_result.json"
...
from kt_epimodel_hira.jax_model.loss_jax import HIRA_AGE_GROUPS, make_multi_season_loss_fn
from kt_epimodel_hira.jax_model.numpyro_model import hira_model, make_ngm_eigvalue_fn
...
def main():
    ...
    print("=" * 70)
    print("M2 production — reparam A + φ fixed 1.0 + γ registry")
    print("=" * 70)
    src = get_active_source()
    print(f"  γ source: {src.key}")
    print(f"  φ:        FIXED at 1.0 (uniform, sample 대상 X — v7b/v8/v9 비식별 확정)")
    print(f"  log_R0:   TN(log 1.5, 0.3, [log 0.8, log 2.8])")
    print(f"  logit_pi: Normal(0, 0.3)")
    print(f"  NUTS:     300 warmup + 300 sample × 4 chain, depth=8, accept=0.8")
    ...
    # Diverse init across 4 chains via stepr0 vectors, but φ now fixed at 1.0
    # so init only sets log_R0 + logit_pi. Reverse-derive using phi_full=ones.
    ...
    with mlflow.start_run(run_name="m2_prod_phi_fixed_reparam"):
        ...
        mcmc.run(random.PRNGKey(7), init_params=init_params,
        ...
        log_R0 = np.asarray(samples["log_R0"])    # (4, 1000, 4)
        logit_pi = np.asarray(samples["logit_pi"])# (4, 1000, 4, 4)
        beta = np.asarray(samples["beta"])        # (4, 1000, 16)
        R0 = np.asarray(samples["R0"])            # (4, 1000, 4)
        pi = np.asarray(samples["pi"])            # (4, 1000, 4, 4)

        # ── Save: netcdf primary, npz fallback (independent try/except each) ──
        ...
        # Posterior summaries
        R0_mean_per_season = R0.mean(axis=(0, 1)).tolist()           # (4,)
        R0_q025 = np.quantile(R0, 0.025, axis=(0, 1)).tolist()
        R0_q975 = np.quantile(R0, 0.975, axis=(0, 1)).tolist()
        pi_mean_per_season = pi.mean(axis=(0, 1)).tolist()           # (4, 4)
        beta_mean = beta.mean(axis=(0, 1)).tolist()
        beta_q025 = np.quantile(beta, 0.025, axis=(0, 1)).tolist()
        beta_q975 = np.quantile(beta, 0.975, axis=(0, 1)).tolist()

        per_chain_R0 = []
        for c in range(4):
            per_chain_R0.append({
                "chain": c, "init": init_names[c],
                "R0_mean_per_season": R0[c].mean(0).tolist(),
            })
        ...
        result = {
            ...
            "R0": {
                "mean_per_season": to_native(R0_mean_per_season),
                "q025": to_native(R0_q025), "q975": to_native(R0_q975),
                "overall_mean": float(R0.mean()),
                "overall_min": float(R0.min()),
                "overall_max": float(R0.max()),
            },
            "pi_mean_per_season": to_native(pi_mean_per_season),
            "beta": {
                "mean": to_native(beta_mean),
                "q025": to_native(beta_q025),
                "q975": to_native(beta_q975),
                "max": float(beta.max()),
            },
            "per_chain_R0": to_native(per_chain_R0),
        ...
        try:
            write_flag(
                SENTINEL,
                f"M2 PROD DONE wall={wall:.0f}s div={n_div} "
                f"r_hat={rhat_max:.3f} ess_min={ess_min:.0f} "
                f"R0_mean={float(R0.mean()):.3f} "
                f"R0_range=[{float(R0.min()):.3f}, {float(R0.max()):.3f}] "
        ...
        print(f"  divergences = {n_div}/2400")
        print(f"  r_hat max = {rhat_max:.3f}    ess_min = {ess_min:.0f}")
        print(f"  R0 per season: {[f'{x:.3f}' for x in R0_mean_per_season]}")
        for c in range(4):
            print(f"  chain {c} ({init_names[c]:>14}) R0 = "
                  f"{[f'{x:.3f}' for x in R0[c].mean(0).tolist()]}")
        print("=" * 60)
        ...
        try:
            ...
            mlflow.log_metric("R0_min", float(R0.min()))
            mlflow.log_metric("R0_max", float(R0.max()))
        except Exception as e:
