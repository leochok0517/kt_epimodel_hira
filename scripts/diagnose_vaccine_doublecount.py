"""Diagnose whether R(0) (vaccine-inclusive) + annual_coverage double-counting
distorts elderly / adult fit and forces φ up as compensation.

Single season 2019-2020, φ = ones(15) FIXED (so that any age-fit distortion
cannot be masked by φ). β_4 (+ phi_nb) is the free parameter, refit under
each condition via L-BFGS multi-start 12 with NB likelihood.

Conditions:
  (a) baseline: R(0) default + v_rate on   (current production)
  (b) v_rate OFF: R(0) default + annual_coverage = 0
      (all vaccine effect assumed inside R(0), no dynamic V transition)
  (c) R(0) with vaccine-share removed + v_rate on:
      R(0) = [0.10×4, 0.30×6, 0.20×3, 0.25×2]
      (i.e. elderly 65+ 0.65 → 0.25 : natural + cross-immunity only,
       50-64 0.45 → 0.20 : natural accumulation only)
      annual_coverage unchanged (dynamic V picks up the vaccine effect)

Measurement per condition:
  1. Elderly (idx 14) S/V/R at t=0, mid, end.  Fraction of S(0) moving to
     V vs R vs remaining.
  2. Best β_4 and NLL under φ=ones(15) fixed.
  3. Per-age r = obs_peak_sum(±2w) / model_peak_sum(±2w) for 6 HIRA groups.

Decision guide (comments only):
- (c) with φ=1 gives r(65+) ≈ 1 → the elder φ-boost we saw before was compen-
  sating for double-counted vaccination, not real susceptibility.
- (a) elderly S depleted heavily, (c) less so → double-count drives elder S loss.
- Adult r shifts across conditions → adult fit also affected.
...
OUT_JSON = REPO_ROOT / "outputs" / "eda" / "vaccine_doublecount.json"
OUT_FIG = REPO_ROOT / "presentations" / "figures" / "vaccine_doublecount.png"
...
# Condition (c) immunity: vaccine-share removed
# elderly 65+ 0.65 → 0.25 (natural + cross-imm), 50-64 0.45 → 0.20 (natural only)
R0_IMMUNITY_C = np.array(
    [0.10] * 4 + [0.30] * 6 + [0.20] * 3 + [0.25] * 2,
    dtype=np.float64,
)
...
def build_setup():
    ...
    # NGM (for R0 reporting; uses default immunity since NGM formula uses
    # S_frac = 1 - immunity)
...
def build_shared(annual_coverage_15: np.ndarray, setup):
    kw = dict(setup["shared_base"])
    kw["annual_coverage"] = jnp.asarray(annual_coverage_15)
    return kw
...
def elder_svr_trajectory(states: jnp.ndarray, age_idx: int = 14) -> dict:
    """Extract S/V/R trajectory for a single age (idx 14 = 70+)."""
    ...
    Sm, Vm, Rm = float(S[mid]), float(V[mid]), float(R[mid])
    Se, Ve, Re = float(S[end]), float(V[end]), float(R[end])
    N0, Ne = float(N[0]), float(N[end])
    ...
    return dict(
        ...
        day_mid=dict(day=mid, S=Sm, V=Vm, R=Rm, S_pct=pct(Sm, N0),
                     V_pct=pct(Vm, N0), R_pct=pct(Rm, N0)),
...
def fit_condition(name: str, immunity_15, annual_coverage_15, setup) -> dict:
    state0 = build_state0(immunity_15, setup)
    shared = build_shared(annual_coverage_15, setup)
    ...
    return dict(
        ...
        annual_coverage_15=[float(x) for x in annual_coverage_15],
...
def main():
    print("=" * 78, flush=True)
    print(f"DIAGNOSE: vaccine double-count effect on elder fit  —  {SEASON_LABEL}",
          flush=True)
    print(f"  free: β_4 + phi_nb   φ = ones(15) FIXED", flush=True)
    print(f"  multi-start {N_STARTS}, NB obs, HOLIDAY on, AMP={AMP}", flush=True)
    print("=" * 78, flush=True)

    setup = build_setup()
    cov_default = np.asarray(setup["vax"].annual_coverage, dtype=np.float64)
    cov_zero = np.zeros(15, dtype=np.float64)

    print(f"  R0_IMMUNITY default (15) = "
          f"{[round(float(x),2) for x in R0_IMMUNITY_PROFILE]}", flush=True)
    print(f"  R0_IMMUNITY (c) vaccine-removed = "
          f"{[round(float(x),2) for x in R0_IMMUNITY_C]}", flush=True)
    print(f"  annual_coverage default = "
          f"{[round(float(x),2) for x in cov_default]}", flush=True)
    print(f"  n_weeks = {setup['n_weeks']}", flush=True)

    conditions = [
        ("a_baseline", R0_IMMUNITY_PROFILE, cov_default),
        ("b_vrate_off", R0_IMMUNITY_PROFILE, cov_zero),
        ("c_R0_vax_removed", R0_IMMUNITY_C, cov_default),
    ]

    all_results = []
    for name, imm, cov in conditions:
        print(f"\n── fit condition [{name}] ──", flush=True)
        r = fit_condition(name, imm, cov, setup)
        ...
        print(f"    ★ elder (idx 14 = 70+): "
              f"S(0)/N={elder['day0']['S_pct']:.2f}% ({elder['day0']['S']:,.0f})",
              flush=True)
        print(f"       day_end S/N={elder['day_end']['S_pct']:.2f}%  "
    ...
    for r in all_results:
        b = r["beta_4"]
        print(f"  {r['condition']:>18s}  {b[0]:.4f}  {b[1]:.4f}  {b[2]:.4f}  "
              f"{b[3]:.4f}  {r['nll']:.4e}  {r['R0_ngm']:.3f}", flush=True)

    print("\n  Elderly (idx 14 = 70+) — S(0)→V vs remaining", flush=True)
    print("  cond               S(0)%   S(end)%  V(end)%  R(end)%  →V%    S rem%", flush=True)
    for r in all_results:
        e = r["elderly_70p_trajectory"]
        print(f"  {r['condition']:>18s}  {e['day0']['S_pct']:5.2f}  "
    ...
    print("  cond               " + "  ".join(f"{ag:>7s}"
                                                for ag in HIRA_AGE_GROUPS),
          flush=True)
    for r in all_results:
        row = f"  {r['condition']:>18s}  "
        for ag in HIRA_AGE_GROUPS:
    ...
    with open(OUT_JSON, "w") as f:
        json.dump({
            ...
            "setup": {
                ...
                "R0_IMMUNITY_C_vax_removed": R0_IMMUNITY_C.tolist(),
    ...
    axes[0].set_title("Elderly 70+ (idx 14): end-of-season S/V/R (% of N)")
    ...
    fig.suptitle(f"Vaccine double-count diagnostic  —  {SEASON_LABEL}  "
                  f"(φ = ones(15) fixed)")
    fig.tight_layout()
