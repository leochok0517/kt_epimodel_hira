...
OUT_JSON = REPO_ROOT / "outputs" / "eda" / "sens_workshare_kappa_v2.json"
OUT_FIG = REPO_ROOT / "presentations" / "figures" / "sens_workshare_kappa_v2.png"
V1_JSON = REPO_ROOT / "outputs" / "eda" / "sens_workshare_kappa.json"
...
# v2 change: school pin tightened 0.15 → 0.05  (h, s, o loose; work strong; school tight)
SIGMA_PER_CHANNEL = np.array([0.15, 0.01, 0.05, 0.15])  # (h, w, s, o)
...
def main():
    ...
    if V1_JSON.exists():
        try:
            ...
            print(f"  [warn] v1 JSON read failed: {e}", flush=True)

    if v1_data is not None:
        v1_lookup = {}
        for rec in v1_data.get("combos", []):
            v1_lookup[(rec["work_share"], rec["kappa"])] = dict(
                sick=rec["policy"]["averted_sick_total_pct"],
                school=rec["policy"]["averted_school_total_pct"],
                pi_school=rec["fit"]["pi"][2],
                nll=rec["fit"]["nll"],
            )
        print("\n  v1 (σ_school=0.15) vs v2 (σ_school=0.05)", flush=True)
        print(f"  {'ws':>5s}  {'κ':>4s}  {'sick v1':>8s}  {'sick v2':>8s}  "
              f"{'Δsick':>7s}   {'schoolπ v1':>10s} {'schoolπ v2':>10s}  "
              f"{'schAvert v1':>12s} {'schAvert v2':>12s}", flush=True)
        for rec in all_results["combos"]:
            key = (rec["work_share"], rec["kappa"])
            v1r = v1_lookup.get(key)
            if v1r is None:
                continue
            v2_sick = rec["policy"]["averted_sick_total_pct"]
            v2_pi_s = rec["fit"]["pi"][2]
            v2_sch = rec["policy"]["averted_school_total_pct"]
            print(f"  {rec['work_share']:>5.2f}  {rec['kappa']:>4.1f}  "
                  f"{v1r['sick']:>+8.2f}  {v2_sick:>+8.2f}  "
                  f"{v2_sick - v1r['sick']:>+7.2f}   "
                  f"{v1r['pi_school']:>10.4f} {v2_pi_s:>10.4f}  "
                  f"{v1r['school']:>+12.2f} {v2_sch:>+12.2f}", flush=True)
