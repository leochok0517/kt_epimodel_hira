"""Phase 4: Multi-start refit with step-function R(0) profile.

Goal: verify that age-specific R(0) [.10/.30/.45/.65] + γ_elder Normal(0.25,0.07)
(via natural NLL fit, no prior penalty) resolves the elderly overprediction
and produces reasonable peak ratios across all 6 HIRA age groups.

Differences vs scripts/multistart_verification.py:
- initial_immunity = R0_IMMUNITY_PROFILE (step) instead of 0.30 uniform
- Output prefix "stepr0_"
- Sentinel flag STEP_R0_DONE.flag for automatic monitoring
- mlflow experiment "hira_calibration_step_r0_refit"

Run detached:
    nohup caffeinate -i -s uv run python scripts/multistart_step_r0.py \\
        > outputs/calibration/multistart_step_r0.log 2>&1 & disown
"""
...
SENTINEL = OUTDIR / "STEP_R0_DONE.flag"
...
def fit_from_start(start_name, start_vec, seasons, outdir_str, mlflow_uri):
    """Single L-BFGS-B fit with step R(0). Parallel-safe."""
    import os
    os.environ["OMP_NUM_THREADS"] = "1"
    os.environ["OPENBLAS_NUM_THREADS"] = "1"
    os.environ["MKL_NUM_THREADS"] = "1"
    ...
    out_path = Path(outdir_str) / f"stepr0_{start_name}.json"
    if out_path.exists():
    ...
    try:
        ...
        for s in seasons:
            ...
            losses[s] = make_loss_function_by_age(
                ...
                initial_immunity=R0_IMMUNITY_PROFILE,   # <-- STEP R(0) (was 0.3)
        ...
        def loss_total(vec):
            ...
            # phi smoothing kept (same as multistart_verification)
            phi_smooth = 0.1 * float(np.sum(np.diff(phi)**2))
            return base + phi_smooth

        mlflow.set_tracking_uri(mlflow_uri)
        mlflow.set_experiment("hira_calibration_step_r0_refit")

        with mlflow.start_run(run_name=f"stepr0_{start_name}"):
            mlflow.log_params({
                "method": "L-BFGS-B step R(0)", "start_point": start_name,
                "lambda_phi_smooth": 0.1, "phi_bound_lower": 0.05,
                "R0_profile": "step [.10/.30/.45/.65]",
                "gamma_prior": "none (natural NLL fit)",
            })

            t0 = time.perf_counter()
            sol = minimize(
                loss_total, start_vec.copy(),
            ...
            phi_pen_f = 0.1 * float(np.sum(np.diff(phi_f)**2))
            ...
            out = {
                ...
                "R0_profile": "step_4_[.10/.30/.45/.65]",
                "phi_lambda_smooth": 0.1,
        ...
        return {"start": start_name, "status": "complete",
                "gamma_elder": float(gamma_f[2]),
                "nll_data": float(nll_data_f), "wall_min": wall / 60}

    except Exception as e:
        err = f"{e}\n{traceback.format_exc()}"
        with open(out_path.parent / f"stepr0_{start_name}_ERROR.json", "w") as f:
...
def main():
    ...
    print("=" * 70)
    print("Phase 4: Step R(0) refit")
    print("  R(0) profile: [.10, .30, .45, .65] (4-step)")
    print("  gamma_elder: natural NLL fit (no prior, bound [0.03, 0.50])")
    print("  8 starts, sequential per worker, parallel n_jobs=8")
    print("=" * 70)

    starts = make_start_points(seed=42)
    t0 = time.perf_counter()
    results = Parallel(n_jobs=len(starts), backend="loky", verbose=10)(
        delayed(fit_from_start)(name, vec, SEASONS, str(OUTDIR), MLFLOW_URI)
    ...
    with open(OUTDIR / "stepr0_summary.json", "w") as f:
        json.dump(summary, f, indent=2, default=str)

    for r in results:
        if r["status"] == "complete":
            print(f"  {r['start']:>14}: gamma_elder={r['gamma_elder']:.4f}  "
                  f"NLL={r['nll_data']:,.0f}  ({r['wall_min']:.0f}min)")
        else:
    ...
    SENTINEL.write_text(
        f"STEP_R0 DONE wall_min={wall_total/60:.1f} "
        f"n_complete={len(completed)}/{len(starts)}\n"
    )
