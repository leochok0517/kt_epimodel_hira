...
$v(t)$ & Vaccine-uptake hazard & $\tfrac{-\ln(1-C)}{Z}\,\mathrm{density}(t)$ & Gaussian density (peak ISO wk $42$, $\sigma{=}4$ wk); $C{=}0.30$, $Z{\approx}0.9332$ \\
\bottomrule
\end{tabular}
\par\vspace{2mm}
...
\end{tabular}}
\par\vspace{2mm}
{\footnotesize\textit{Note.} $\phi_a$: three literature anchors (0--4 $=2.0$; 20--44 $=1.0$; 70+ $=1.5$) linearly interpolated \cite{Cauchemez2009}. $\gamma_{\mathrm{report},a}$: CDC-style \cite{Reed2015}, $15$--$19$ set to adult level. $\kappa_a$: student $0.34$ (ages 0--19), adult $0.40$ (ages 20--69), $\kappa_{70+}=0$; co-presence discount excluded. $^{\dagger}\rho_a$ (employment rate, Statistics Korea) is $0$ for student bands and rises to $\approx 0.74$; students take $\rho_a=0$ (no commute), so $\kappa$ acts on the home channel only.}
\end{table}
