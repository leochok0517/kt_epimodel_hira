...
#### 2.2.7 φ 추가 좁힘 — v7b 비식별 실증 후 (LogNormal σ 0.05)

**최종 prior (v7b 이후)**: `φ ~ LogNormal(0, 0.05)` (φ_5≡1.0 anchor 유지)

| 항목 | 값 | 근거 |
|---|---|---|
| 중앙값 | 1.0 (exp 0) | reference normalization |
| σ | **0.05** | v7b 비식별 실증 + Cauchemez ±15% conservative |
| ≈95% mass | [0.91, 1.10] | ±10% (Cauchemez 보다 보수) |

**v7b 실증** (4 chain × 300 iter, R(0) 계단 + γ 고정 + reparam A):
- φ posterior **chain별 mode 제각각**: φ[0] = 4.90 / 5.73 / 6.37 / 6.62
- φ 전체: mean 3.17, range **[0.013, 9.61]**, prior(median 1.0) 무시 67%
- **r_hat 4.46, ess 5** — φ 차원은 사실상 비식별
- R0 는 4 chain 정합 [1.87, 2.13] (식별 OK) → 비식별은 φ 만의 문제

**결론**: 데이터가 φ 에 정보 없음 확정 → prior 가 사실상 결정.
σ 0.15 → **0.05** 좁힘. "고정" 아니라 "강한 문헌 prior" (정직).

**slide O 표 정합**:
- γ: 정보 없음 → 외부 고정 (CDC + PSA)
- **φ: 약함 → prior 좁게 (σ=0.05)** ← v7b 가 이 행을 실증
- β: 강함 → 데이터 추정 (R0 reparam)

#### 2.2.8 φ 최종 처리 — 점고정 1.0 (v7b/v8/v9 3중 확인 후)

**최종 결정**: `φ_a ≡ 1.0` (전 연령 균등, sample 대상에서 제외)

**3 중 비식별 증거**:

| smoke | 설정 | 결과 |
|---|---|---|
| v7b | reparam A + φ LogNormal σ=0.15, 다양 init | r_hat 4.46, ess 5, chain φ [4.9, 6.6] |
| v8 | + σ 0.15→0.05 (3× 좁힘) | r_hat 4.43 (변화 없음), chain φ [1.7, 2.4] |
| v9 | + 통일 init (φ=1, R0=1.5, π=균등) | r_hat 3.39, chain φ overall [1.75, 2.29] |

세 smoke 공통:
- divergence 0
- **R0 4 chain 정합** (range [1.87, 2.23]) — β·R0 식별 확정
- **φ chain 별 갈라짐** — prior 좁힘·init 통일 둘 다 실패

→ φ 는 데이터로 분해 불가 (모델/데이터 구조적). prior 외부 고정으로 차원 제거.

**φ=1.0 (균등) 선택 근거**:
- 연령 전파차는 **4채널 contact + 계단 R(0)** 가 이미 담당
- φ 는 그 위 per-contact 보정인데 데이터 정보 없고 문헌상 작음 (±15%)
- 균등 1.0 = 가장 단순 + 중립 + φ_5 anchor 와 자동 일관

**φ 고정값 민감도 검증** (φ=1.0 vs Cauchemez 패턴, 같은 R0 target):
- R0 정확히 동일 (1.95 / 2.05 / 1.92 / 1.95)
- NLL 차이 0.84% (multi-start 잡음 0.57% 와 같은 수준) → φ 형태 무관
- 즉 균등 1.0 선택이 다른 합리적 선택과 결과 동치

**차원 변화**: 30 (R0 4 + simplex 12 + φ 14) → **16 (R0 4 + simplex 12)** — φ 14 제거
